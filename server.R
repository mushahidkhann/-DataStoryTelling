...
################################################################################
  url='https://s3-ap-southeast-1.amazonaws.com/he-public-data/covid196c95c6e.json'
  df = fromJSON(url, flatten=T)
  COVID_table_data = data.frame(df,stringsAsFactors = F)
  output$COVID_table <- DT::renderDataTable({
    action <- DT::dataTableAjax(session, COVID_table_data)
    DT::datatable(
      COVID_table_data,
      options = list(
        searching = T,
